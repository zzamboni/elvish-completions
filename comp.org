...
*Note:* the algorithm used at the moment for identifying command-line options only works correctly with options which start with a dash (=-=) and which do not have space-separated values. This will be fixed in an upcoming version.
