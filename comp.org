...
=comp:sequence= receives a definition map and the current contents of the command line. The first element of =$cmd= is the command, and it should not be included in the definition. If =$def= contains a key =-opts=, it is expanded and interpreted as a map of option definitions for =edit:complete-getopt=. If =-opts= produces a list of strings, they are considered as long option names and converted to the appropriate data structure.
